...
const S = {
  ...
  text40:   { color: 'rgba(255,255,255,0.40)' },
  text50:   { color: 'rgba(255,255,255,0.50)' },
  text25:   { color: 'rgba(255,255,255,0.25)' },
  text20:   { color: 'rgba(255,255,255,0.20)' },
  text80:   { color: 'rgba(255,255,255,0.80)' },
  text85:   { color: 'rgba(255,255,255,0.85)' },
}
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4"
      ...
      <div className="w-full max-w-2xl rounded-2xl flex flex-col overflow-hidden"
        style={{ background: 'rgba(14,14,26,0.98)', border: '1px solid rgba(255,255,255,0.10)', maxHeight: '88dvh', boxShadow: '0 32px 80px rgba(0,0,0,0.7)' }}>
        {/* Header */}
        <div className="flex items-center justify-between px-5 py-4" style={{ borderBottom: '1px solid rgba(255,255,255,0.07)' }}>
          <div className="flex items-center gap-2.5">
            <div className="w-7 h-7 rounded-lg flex items-center justify-center" style={{ background: 'rgba(255,0,0,0.15)', border: '1px solid rgba(255,0,0,0.25)' }}>
            ...
            {!isHost && <span className="text-xs px-2 py-0.5 rounded-full" style={{ background: 'rgba(139,92,246,0.15)', color: '#a78bfa', border: '1px solid rgba(139,92,246,0.25)' }}>سيُضاف للطابور</span>}
          </div>
          <button onClick={onClose} className="w-8 h-8 flex items-center justify-center rounded-lg" style={{ background: 'rgba(255,255,255,0.06)', color: 'rgba(255,255,255,0.60)' }}>
            ...
            <>
                  ...
                  {results.map((v) => (
                    <div key={v.id} className="rounded-xl overflow-hidden"
                      ...
                      <div className="relative" style={{ aspectRatio: '16/9' }}>
                        ...
                        {v.duration && (
                          <span className="absolute bottom-1 right-1 px-1 rounded text-[10px] font-bold text-white" style={{ background: 'rgba(0,0,0,0.75)' }}>{v.duration}</span>
                        )}
                      </div>
                      <div className="p-2.5">
                        <p className="text-sm text-white font-medium leading-tight line-clamp-2 mb-1">{v.title}</p>
                        <p className="text-xs mb-2.5" style={{ color: 'rgba(255,255,255,0.40)' }}>{v.channel}</p>
              ...
              {results.length === 0 && !loading && (
                <div className="flex flex-col items-center justify-center py-12 gap-3">
                  <div className="w-14 h-14 rounded-2xl flex items-center justify-center" style={{ background: 'rgba(255,0,0,0.08)', border: '1px solid rgba(255,0,0,0.15)' }}>
                    <svg width="28" height="28" viewBox="0 0 24 24" fill="none">
                      <path d="M22.54 6.42a2.78 2.78 0 0 0-1.95-1.96C18.88 4 12 4 12 4s-6.88 0-8.59.46a2.78 2.78 0 0 0-1.95 1.96A29 29 0 0 0 1 12a29 29 0 0 0 .46 5.58A2.78 2.78 0 0 0 3.41 19.6C5.12 20 12 20 12 20s6.88 0 8.59-.46a2.78 2.78 0 0 0 1.95-1.95A29 29 0 0 0 23 12a29 29 0 0 0-.46-5.58z" fill="rgba(255,68,68,0.5)"/>
                      <polygon points="9.75 15.02 15.5 12 9.75 8.98 9.75 15.02" fill="rgba(255,255,255,0.5)"/>
                    </svg>
                  </div>
                  <p style={{ color: 'rgba(255,255,255,0.40)' }} className="text-sm">ابحث عن أي مقطع على يوتيوب</p>
                </div>
              )}
            </>
          )}
  ...
  if (queue.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-6 gap-2 text-center">
        ...
        <p className="text-xs" style={{ color: 'rgba(255,255,255,0.25)' }}>الطابور فارغ</p>
      </div>
    )
  }
  ...
  return (
    <div className="space-y-2">
      ...
      {queue.map((item, idx) => {
        ...
        return (
          <div key={item.id} className="flex gap-2.5 p-2.5 rounded-xl group"
            style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.07)' }}>
            {/* Rank badge */}
            <div className="w-5 flex-shrink-0 flex items-start justify-center pt-0.5">
              <span className="text-[10px] font-bold" style={{ color: 'rgba(255,255,255,0.25)' }}>#{idx + 1}</span>
            </div>

            {/* Thumbnail */}
            {item.thumbnail && item.source === 'youtube' && (
              <div className="flex-shrink-0 rounded-md overflow-hidden" style={{ width: 56, height: 32 }}>
                {/* eslint-disable-next-line @next/next/no-img-element */}
                <img src={item.thumbnail} alt={item.title} className="w-full h-full object-cover" />
              </div>
            )}
            {item.source === 'drive' && (
              <div className="flex-shrink-0 w-14 h-8 rounded-md flex items-center justify-center" style={{ background: 'rgba(255,255,255,0.06)' }}>
            ...
            <div className="flex-1 min-w-0">
              <p className="text-xs text-white font-medium line-clamp-2 leading-tight">{item.title || item.videoId}</p>
              <p className="text-[10px] mt-0.5" style={{ color: 'rgba(255,255,255,0.35)' }}>بواسطة {item.addedByName}</p>
            </div>
            ...
            <div className="flex flex-col items-center gap-1 flex-shrink-0">
              ...
              <button onClick={() => onVote(item.id)}
                ...
                style={hasVoted
                  ...
                  : { background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.08)', color: 'rgba(255,255,255,0.40)' }
                }>
                <svg width="10" height="10" viewBox="0 0 24 24" fill={hasVoted ? 'currentColor' : 'none'} stroke="currentColor" strokeWidth="2.5">
                  <path d="M12 19V5M5 12l7-7 7 7"/>
                </svg>
                <span className="text-[10px] font-bold leading-none">{item.votes.length}</span>
              </button>

              {/* Remove (owner or host) */}
              {(isOwner || isHost) && (
...
export default function RoomPage() {
  ...
  useEffect(() => {
    ...
    socket.on('play', (p: any) => { setRoom((r) => r ? { ...r, isPlaying: true, currentTime: p.currentTime } : r); videoRef.current?.play() })
    socket.on('pause', (p: any) => { setRoom((r) => r ? { ...r, isPlaying: false, currentTime: p.currentTime } : r); videoRef.current?.pause() })
    socket.on('seek', (p: any) => { setRoom((r) => r ? { ...r, currentTime: p.currentTime } : r); if (videoRef.current) videoRef.current.currentTime = p.currentTime })
    socket.on('change_video', (p: any) => setRoom((r) => r ? { ...r, currentVideo: p.videoId, currentSource: p.source as VideoSource, currentTime: 0 } : r))
  ...
  useEffect(() => {
    ...
    heartbeatInterval.current = setInterval(() => {
      socket.emit('heartbeat', { currentTime: videoRef.current?.currentTime ?? 0, buffered: 0, latency: 0 })
    }, 5000)
  ...
  const handleYouTubeQueue = useCallback((id: string, title: string, thumbnail: string) => {
    ...
    const item: QueueItem = {
      ...
      votes: [session.user.id], // auto-vote for your own item
    }
  ...
  function handleQueueVote(id: string) {
    socket?.emit('queue_vote', { id } as any)
  }

  function handleQueueRemove(id: string) {
    socket?.emit('queue_remove', { id } as any)
  }

  function handleQueueSkip() {
    socket?.emit('queue_skip' as any)
  }
    ...
    <div className="flex flex-col md:flex-row h-[100dvh] bg-background overflow-hidden">
      ...
      <div className={`flex flex-col min-w-0 ${showChat ? 'hidden md:flex md:flex-1' : 'flex flex-1 md:flex-1'}`}>
        ...
        <header className="flex items-center justify-between px-4 py-3 flex-shrink-0"
          style={{ backdropFilter: 'blur(20px)', background: 'rgba(0,0,0,0.25)', borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
          <div className="flex items-center gap-2.5">
            <button onClick={() => router.push('/')} className="btn-ghost p-2 -ml-2 text-white/50 hover:text-white" title="Leave Room">
            ...
            <div className="flex items-center gap-1.5">
              <span className="font-bold text-white text-sm hidden sm:inline">Watch Party</span>
              <span className="text-sm hidden sm:inline" style={S.text40}>/</span>
              <span className="text-sm font-mono" style={S.text50}>{slug}</span>
          ...
          <div className="flex items-center gap-2">
            <div className="flex items-center gap-1.5">
              ...
              <span className="text-xs hidden sm:inline" style={S.text40}>{connected ? 'Live' : 'Offline'}</span>
            </div>
            ...
            <button className="md:hidden btn-ghost p-1.5 relative" onClick={() => setShowChat(true)} title="Open Chat">
              ...
              {queue.length > 0 && <span className="absolute -top-0.5 -right-0.5 w-4 h-4 rounded-full text-[9px] font-bold flex items-center justify-center" style={{ background: '#7c3aed', color: 'white' }}>{queue.length}</span>}
            </button>
            {/* Logout */}
            {session && (
              <button id="logout-room-btn" onClick={() => signOut({ callbackUrl: '/login' })} title="Sign Out"
                className="w-8 h-8 flex items-center justify-center rounded-lg transition-all duration-200"
                style={{ background: 'rgba(239,68,68,0.10)', border: '1px solid rgba(239,68,68,0.22)', color: 'rgba(239,68,68,0.70)' }}>
            ...
            {session && <div className="w-7 h-7 avatar text-xs hidden sm:flex">{session.user.name?.[0]?.toUpperCase()}</div>}
          </div>
        </header>
        ...
        <div ref={videoContainerRef} className="flex-1 bg-black flex items-center justify-center relative overflow-hidden group">
          {!room?.currentVideo && (
            <div className="text-center space-y-4 px-6">
              <div className="w-20 h-20 rounded-2xl flex items-center justify-center mx-auto"
                ...
                <svg width="36" height="36" viewBox="0 0 24 24" fill="none" stroke="rgba(255,255,255,0.18)" strokeWidth="1.5"><polygon points="5 3 19 12 5 21 5 3"/></svg>
              </div>
              <div>
                <p className="text-lg font-medium" style={S.text40}>No video selected</p>
                <p className="text-sm mt-1" style={S.text25}>
          ...
          {room?.currentVideo && room.currentSource === 'youtube' && (
            <iframe ref={playerRef}
              src={`https://www.youtube.com/embed/${room.currentVideo}?enablejsapi=1&start=${Math.floor(room.currentTime)}&autoplay=1`}
              className={`w-full h-full ${!room.isHost ? 'pointer-events-none' : ''}`} allow="autoplay; fullscreen" allowFullScreen />
          )}
          {room?.currentVideo && room.currentSource === 'drive' && (
            <video key={room.currentVideo} ref={videoRef}
              ...
              autoPlay onPlay={handlePlay} onPause={handlePause} onSeeked={handleSeeked} />
          )}
          {/* Fullscreen button */}
          <button onClick={toggleFullscreen} title={isFullscreen ? 'تصغير' : 'تكبير'}
            className="absolute bottom-3 right-3 w-9 h-9 flex items-center justify-center rounded-xl opacity-0 group-hover:opacity-100 transition-all duration-200 z-10"
            style={{ background: 'rgba(0,0,0,0.55)', border: '1px solid rgba(255,255,255,0.15)', color: 'white', backdropFilter: 'blur(8px)' }}>
            {isFullscreen
              ? <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round"><path d="M8 3v3a2 2 0 0 1-2 2H3m18 0h-3a2 2 0 0 1-2-2V3m0 18v-3a2 2 0 0 1 2-2h3M3 16h3a2 2 0 0 1 2 2v3"/></svg>
        ...
        {room?.isHost && (
          <div className="flex-shrink-0 p-3 md:p-4"
            style={{ backdropFilter: 'blur(20px)', background: 'rgba(0,0,0,0.20)', borderTop: '1px solid rgba(255,255,255,0.06)' }}>
            <div className="flex gap-2">
              <button id="youtube-picker-btn" onClick={() => setShowYouTubeModal(true)}
                className="flex items-center gap-2 px-3 md:px-4 py-2.5 rounded-xl font-medium text-sm flex-1 transition-all duration-200"
                style={{ background: 'rgba(255,0,0,0.10)', border: '1px solid rgba(255,0,0,0.22)', color: 'rgba(255,120,120,0.9)' }}>
                <svg width="16" height="16" viewBox="0 0 24 24" fill="none">
                  <path d="M22.54 6.42a2.78 2.78 0 0 0-1.95-1.96C18.88 4 12 4 12 4s-6.88 0-8.59.46a2.78 2.78 0 0 0-1.95 1.96A29 29 0 0 0 1 12a29 29 0 0 0 .46 5.58A2.78 2.78 0 0 0 3.41 19.6C5.12 20 12 20 12 20s6.88 0 8.59-.46a2.78 2.78 0 0 0 1.95-1.95A29 29 0 0 0 23 12a29 29 0 0 0-.46-5.58z" fill="rgba(255,80,80,0.85)"/>
                  <polygon points="9.75 15.02 15.5 12 9.75 8.98 9.75 15.02" fill="white"/>
                </svg>
                <span>YouTube</span>
              </button>
        ...
        {/* Non-host: add to queue button */}
        {!room?.isHost && room !== null && (
          <div className="flex-shrink-0 px-3 pb-3 pt-2"
            style={{ backdropFilter: 'blur(20px)', background: 'rgba(0,0,0,0.18)', borderTop: '1px solid rgba(255,255,255,0.06)' }}>
      ...
      <aside
        ...
        style={{ borderLeft: '1px solid rgba(255,255,255,0.06)', background: 'rgba(8,8,16,0.95)' }}>

        {/* Mobile header */}
        <div className="flex items-center justify-between px-4 py-3 md:hidden flex-shrink-0"
          style={{ borderBottom: '1px solid rgba(255,255,255,0.07)', background: 'rgba(0,0,0,0.40)' }}>
          <span className="text-sm font-semibold" style={{ color: 'rgba(255,255,255,0.80)' }}>Watch Party</span>
        ...
        <div className="p-4 flex-shrink-0" style={{ borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
          <div className="flex items-center justify-between mb-3">
            <h3 className="text-xs font-semibold uppercase tracking-wider" style={S.text80}>Participants</h3>
          ...
          <div className="space-y-2 max-h-24 overflow-y-auto scrollbar-thin">
            {participants.map((p) => (
              <div key={p.user.id} className="flex items-center gap-2.5">
                <div className="w-7 h-7 avatar text-xs">{p.user.name?.[0]?.toUpperCase()}</div>
                <div className="flex-1 min-w-0"><p className="text-sm truncate" style={S.text80}>{p.user.name}</p></div>
            ...
            {participants.length === 0 && <p className="text-xs" style={S.text25}>No participants yet...</p>}
          </div>
        </div>

        {/* Tab switcher: Chat | Queue */}
        <div className="flex flex-shrink-0" style={{ borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
          {(['chat', 'queue'] as const).map((t) => (
            <button key={t} onClick={() => setSidebarTab(t)}
              ...
              {sidebarTab === t && <span className="absolute bottom-0 left-0 right-0 h-0.5 rounded-full" style={{ background: '#7c3aed' }} />}
            </button>
          ))}
        </div>
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-3 scrollbar-thin">
              {messages.length === 0 && (
                <div className="flex flex-col items-center justify-center h-full text-center space-y-2 py-8">
                  <svg width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="rgba(255,255,255,0.10)" strokeWidth="1.5">
                    <path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"/>
                  </svg>
                  <p className="text-xs" style={S.text20}>No messages yet. Say hi!</p>
                </div>
              )}
              {messages.map((msg) => {
                ...
                return (
                  <div key={msg.id} className={`flex gap-2 ${isOwn ? 'flex-row-reverse' : 'flex-row'}`}>
                    <div className="w-6 h-6 avatar text-xs self-end flex-shrink-0">{msg.userName?.[0]?.toUpperCase()}</div>
                    <div className={`max-w-[80%] space-y-0.5 flex flex-col ${isOwn ? 'items-end' : 'items-start'}`}>
                      <span className="text-xs px-1" style={{ color: 'rgba(255,255,255,0.35)', textAlign: isOwn ? 'right' : 'left' }}>
                        {isOwn ? 'You' : msg.userName}
                      </span>
                      <div className="px-3 py-2 rounded-2xl text-sm leading-relaxed"
                        style={isOwn
                          ? { background: '#7c3aed', color: 'white', borderBottomRightRadius: '4px' }
                          : { background: 'rgba(255,255,255,0.07)', color: 'rgba(255,255,255,0.85)', border: '1px solid rgba(255,255,255,0.08)', borderBottomLeftRadius: '4px' }
                        }>
                        {msg.message}
                      </div>
                    </div>
              ...
              {typingUsers.length > 0 && (
                <div className="flex items-center gap-2">
                  <div className="w-6 h-6 avatar text-xs flex-shrink-0" style={{ opacity: 0.5 }}>{typingUsers[0]?.[0]?.toUpperCase()}</div>
                  <div className="px-3 py-2 rounded-2xl rounded-bl-sm" style={{ background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.06)' }}>
                    <div className="flex gap-1 items-center h-4">
                      {[0, 150, 300].map((delay) => (
                        <span key={delay} className="w-1.5 h-1.5 rounded-full bg-white opacity-40 animate-bounce" style={{ animationDelay: `${delay}ms` }} />
                      ))}
                    </div>
                  </div>
                </div>
            ...
            <div className="p-3 flex-shrink-0" style={{ borderTop: '1px solid rgba(255,255,255,0.06)' }}>
              <form onSubmit={sendMessage} className="flex gap-2">
                <input id="chat-input" value={chatInput} onChange={(e) => onTyping(e.target.value)}
                  placeholder="Send a message..." className="input-field flex-1 py-2.5" />
                <button id="send-message-btn" type="submit" disabled={!chatInput.trim()}
                  className="w-10 h-10 flex items-center justify-center rounded-xl transition-all duration-200 flex-shrink-0 disabled:opacity-40"
                  style={{ background: '#7c3aed' }}>
